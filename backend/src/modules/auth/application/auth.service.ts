...
export class AuthService {
	...
	async generateAccessToken(id: string, username: string) {
		return this.authDomain.generateAccessToken(username, id);
	}
	...
	async generateTokens(id: string, username: string) {
		...
		const { refreshToken, cookie } =
			await this.authDomain.generateRefreshTokenWithCookie(username, id);
	...
	async login({ username }: LoginUserParameters) {
		...
		const { accessToken, cookie } = await this.generateTokens(
			user.id,
			user.username
		);
	...
	async refreshAccessToken(
		...
		const user = await this.usersService.getUserByIdAndRefreshToken(
			refreshToken,
			userId
		);
	...
	async validateUser({
		...
		const user = await this.usersService.getUserByUsernameWithPassword(
			username
		);
		if (user) {
